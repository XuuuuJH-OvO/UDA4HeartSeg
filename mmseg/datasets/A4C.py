...
class A4CDataset(CustomDataset):
    """Cityscapes dataset.
    ...
    CLASSES = ('Bacground', 'Left_ventricle', 'Myocardium')  #'Myocardium'

    PALETTE = [(0, 0, 0), (128, 128, 128), (255, 255, 255)] #(0,255,0)
    ...
    def __init__(self,
        ...
        super(A4CDataset, self).__init__(
    ...
    def pre_pipeline(self, results):
        super(A4CDataset, self).pre_pipeline(results)
    ...
    def results2img(self, results, imgfile_prefix, to_label_id):
        ...
        for idx in range(len(self)):
            ...
            output.save(png_filename, transparency=None)
    ...
    def evaluate(self,
        ...
        if 'A4C' in metrics:
            eval_results.update(
                self._evaluate_A4C(results, logger, imgfile_prefix))
            metrics.remove('A4C')
        if len(metrics) > 0:
            eval_results.update(
                super(A4CDataset,
    ...
    def _evaluate_A4C(self, results, logger, imgfile_prefix):
        ...
        for seg_map in mmcv.scandir(
                self.ann_dir, '_labelIds.png', recursive=True):
            seg_map_list.append(osp.join(self.ann_dir, seg_map))
            pred_list.append(CSEval.getPrediction(CSEval.args, seg_map))
        # 这里计算最后指标的时候，可以使用对pred_list里的标签做后处理，再观测结果
        eval_results.update(
